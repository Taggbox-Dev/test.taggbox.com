var ApplicationEmbed = ApplicationEmbed || function (w, u) {
	var wallObject = {},
		...
		AppPath = "https://test.taggbox.com/display/";
	var loading = true;
		...
		init: function () {
			...
			for (var i = 0; i < container.length; i++) {
				...
				if (iframe.length == 0) {
					...
					/* for Height */
					var parentElementHeight = container[i].parentElement.parentElement.clientHeight;

					// if (containerHeight > 100) {
					// 	fixedHeight = 0;
					// 	iframeHeight = containerHeight;
					// }
					if (containerHeight > 100 && parentElementHeight == 0) {
						fixedHeight = 0;
						iframeHeight = containerHeight;
					}
					else if (containerHeight === 100 && parentElementHeight != 0) {
						fixedHeight = 0;
						iframeHeight = parentElementHeight;
					}
					else if (containerHeight == parentElementHeight) {
						fixedHeight = 0;
						iframeHeight = containerHeight;
					}
					...
					var wallKey = "";
					var viewUrl = container[i].getAttribute("view-url"); //window.location.href
					var viewUrlWallKey = new URL(viewUrl);
					console.log("viewUrlWallKey", viewUrlWallKey)
					if (viewUrlWallKey.searchParams.has("wallKey") && viewUrlWallKey.searchParams.get("wallKey") && viewUrlWallKey.searchParams.get("wallKey").length > 0) {
						console.log("check", viewUrlWallKey.searchParams.get("wallKey"))
					...
					var iframeElem = '<iframe src="' + AppPath + 's/' + wallUrl + frameUrl + '" data-fixed-height="' + fixedHeight + '" data-height="' + iframeHeight + '" data-wall="' + wallUrl + '" data-position="' + i + '" id="iframe' + r + '" style="visibility: hidden;position: absolute;margin:0;left: -999em;display: inline-block;border: none;width:100%;height:600px;" scrolling="no" frameborder="0" allowtransparency="true" title="Taggbox widget"></iframe>';
